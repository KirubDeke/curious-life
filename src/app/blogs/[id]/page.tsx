  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const formattedComments = commentsRes.data.comments.map(
          (comment: Comment) => ({
            ...comment,
            id: comment.id,
      ...
      <div className="w-full bg-background text-foreground max-w-screen-xl mx-auto px-4 md:px-6 py-8">
        ...
        <div className="container mx-auto px-4 pb-12 mt-12">
          <div className="mb-8 flex items-center">
            <button
              onClick={handleLike}
              className={`relative overflow-hidden px-6 py-3 rounded-full flex items-center space-x-2 transition-all duration-300 ${
                hasLiked
                  ? "bg-blue-100 text-blue-600 shadow-inner"
                  : "bg-blue-50 hover:bg-blue-100 text-blue-500 shadow-sm"
              } group`}
            >
